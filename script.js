...
function isLeapYear(value) {
	if (value % 400 == 0) return 29;
	// century leap year like 2000 or 1600 (1700 is not leap year, yet can devide by 4)
	else if (value % 100 != 0 && value % 4 == 0) return 29;
	//leap year, but not century one, like 2020
	else return 28;
}
...
var timer = setInterval(function () {
	...
	if ($(tag).text().indexOf('Today') == -1) {
		...
		let days = Math.trunc(daysLeft / (1000 * 3600 * 24));
		let hours = Math.trunc((daysLeft % (1000 * 3600 * 24)) / (1000 * 3600));
		let minutes = Math.trunc((daysLeft % (1000 * 3600)) / (1000 * 60));
		let seconds = Math.trunc((daysLeft % (1000 * 60)) / 1000);
